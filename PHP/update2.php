...
function Get_nmber()
{
		...
		global $dbu;
		global $dbp;
		$mysql2 = mysql_connect($dbl,$dbu,$dbp);
		if (!$mysql2)
		{
			 die('出错信息1： ' . mysql_error());
		}
		mysql_query("SET NAMES 'utf8'");
		mysql_select_db("qdm165423358_db",$mysql2);
		$mysql2_counts = mysql_query("select count(*) from sheet1");
		$ls = mysql_fetch_row($mysql2_counts);
		...
		for(;$id <= 2564;$id++)
		{
			...
			mysql_query("SET NAMES 'utf8'");
			mysql_select_db("qdm165423358_db",$mysql2);
			$ls_AREAID_query = mysql_query("select AREAID from sheet1 where ID = $id",$mysql2);
	...
	function Update_db($Weather_Array_data)
	{
		...
		mysql_select_db("qdm165423358_db",$mysql2);
				...
				mysql_query($is_ico_0,$mysql2);
				mysql_query($is_0,$mysql2);
				mysql_query($is_ico_1,$mysql2);
				mysql_query($is_1,$mysql2);
				mysql_query($is_ico_2,$mysql2);
				mysql_query($is_2,$mysql2);
				mysql_close($mysql2);
				return 1;	
	}
?>