...
function Get_nmber()
{
		...
		global $dbu;
		global $dbp;
		$mysql1 = mysql_connect($dbl,$dbu,$dbp);
		if (!$mysql1)
		{
			 die('出错信息1： ' . mysql_error());
		}
		mysql_query("SET NAMES 'utf8'");
		mysql_select_db("qdm165423358_db",$mysql1);
		$mysql1_counts = mysql_query("select count(*) from sheet1");
		$ls = mysql_fetch_row($mysql1_counts);
		...
		for(;$id <= 1999;$id++)
		{
			...
			mysql_query("SET NAMES 'utf8'");
			mysql_select_db("qdm165423358_db",$mysql1);
			$ls_AREAID_query = mysql_query("select AREAID from sheet1 where ID = $id",$mysql1);
	...
	function Update_db($Weather_Array_data)
	{
		...
		mysql_select_db("qdm165423358_db",$mysql1);
				...
				mysql_query($is_ico_0,$mysql1);
				mysql_query($is_0,$mysql1);
				mysql_query($is_ico_1,$mysql1);
				mysql_query($is_1,$mysql1);
				mysql_query($is_ico_2,$mysql1);
				mysql_query($is_2,$mysql1);
				mysql_close($mysql1);
				return 1;	
	}
?>